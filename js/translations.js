export const translations = {
    fr: {
        hub: {
            greeting_pre: "Bienvenue à Ohana Home",
            greeting_sub: "Votre guide personnel pour un séjour inoubliable",
            nav: {
                stay: "Votre séjour",
                addresses: "Nos adresses",
                guestbook: "Livre d'or",
                story: "Notre histoire"
            },
            wifi_label: "WIFI"
        },
        common: {
            back: "Retour",
            home: "Accueil"
        },
        screensaver: {
            prompt: "Touchez l'écran pour commencer",
            ben_slide: {
                text_main: "\"Aujourd'hui, mon bureau n'a plus de murs. J'accompagne les entrepreneurs à trouver leur propre liberté.\n\nJ'ai conçu ce lieu pour qu'il vous inspire la vôtre.\"",
                more_info: "PLUS D'INFOS ICI",
                qr_sub: "Et oui, je fais ma pub 😂",
                sig_name: "BENJAMIN",
                sig_role: "Votre Hôte"
            },
            dog_slide: {
                title: "Salut les humains ! 🐾",
                intro: "Si vous avez adoré votre séjour, un joli <b>5/5</b> c'est comme une grattouille derrière l'oreille : ça nous fait trop plaisir !",
                secret_title: "🤫 Mon petit secret :",
                secret_content: "On pourrait croire que <b>4/5</b> c'est une bonne note... mais pour le méchant algorithme Airbnb, c'est une petite claque sur le museau !<br><br>Il faut environ <b>10 notes parfaites</b> pour rattraper un seul 4/5. Si on en a trop, pfiou... on disparaît des recherches. Plus de visibilité, plus d'invités, plus de croquettes... 🥺",
                cta: "Alors si tout a été au poil, le 5/5 est le meilleur moyen de nous sauver !",
                feedback: "<strong>Un truc qui ne va pas ?</strong> Dites-le à Papa (Benjamin) par message. Il court plus vite que moi pour régler les soucis ! Et à Ohana Home on veut vous chouchouter par ce que vous êtes les meilleurs voyageurs ! ❤️"
            }
        },
        stay: {
            title: "Votre séjour",
            menu: {
                checkout: "Consignes de départ",
                parking: "Parking & Wi-Fi",
                equip: "Équipements",
                rules: "Règles de vie",
                help: "Besoin d'aide ?"
            }
        },
        parking: {
            title: "Parking & Wi-Fi",
            wifi_label: "WIFI",
            hero_label: "Parking Privé",
            hero_desc_remote: "🎮 Télécommande fournie (⚠️ ne pas perdre)",
            hero_desc_access: "Accès : Étage -1 → Sortie → Droite → Droite → Porte Orange (clé rouge)"
        },
        equip: {
            title: "Équipements",
            kitchen: {
                title: "Cuisine",
                oven: "Four : Éteindre et nettoyer après usage",
                dishwasher: "Lave-vaisselle : Maintenir Power 3s. Rouge au sol = ON",
                fridge: "Frigo : Bien fermer, ne pas claquer",
                coffee: "Café : Nespresso + Dolce Gusto",
                cleaning: "Entretien : Éponges magiques fournies (sur blanc)"
            },
            bath: {
                title: "Salle de Bain",
                vent: "Condensation : Ventiler au maximum",
                door: "Laisser la porte ouverte pendant la douche si besoin"
            },
            laundry: {
                title: "Linge",
                towels: "1 Grande + 1 Petite serviette / pers.",
                washer: "Lave-linge : Mode Délicat. Vider bac à eau.",
                dryer: "Sèche-linge : Au-dessus du lave-linge",
                basket: "Panier : SDB, sous le meuble"
            },
            living: {
                title: "Salon & Fenêtres",
                sofa: "Laisser les plaids de protection sur le canapé",
                windows: "Fenêtres : Fragiles (simple vitrage). Manipuler avec soin.",
                curtains: "Rideaux : Isolation phonique + Occultation.",
                shutters: "<strong>Volets :</strong> Rustiques (remplacement prévu cette année). Pour qu'ils ne remontent pas seuls, bloquez la chaîne avec le petit crochet."
            },
            comfort: {
                title: "Confort & Chauffage",
                heater: "<strong>Chauffage d'appoint :</strong> Disponible dans le dressing si besoin.",
                contact: "<strong>Température :</strong> Au moindre problème, appelez-nous ! On veut que vous soyez bien."
            }
        },
        checkout: {
            title: "Consignes Départ",
            before: "Départ avant",
            cleaning_time: "(Ménage à 11h15)",
            steps: {
                laundry: "Linge Sale",
                laundry_sub: "Draps/Serviettes au sol (Cuisine)",
                trash: "Poubelles",
                trash_sub: "Sortir (Face garage)",
                fridge: "Frigo",
                fridge_sub: "Vider (Laisser non-périssables)",
                dishes: "Vaisselle",
                dishes_sub: "Lancer lave-vaisselle",
                power: "Électricité",
                power_sub: "Tout éteindre",
                windows: "Fermer",
                windows_sub: "Fenêtres & Portes",
                keys: "Clés + Badge",
                keys_loc: "Dans la boîte n°2",
                keys_code: "Code ou Clé \"POULET Benjamin\""
            }
        },
        help: {
            title: "Besoin d'aide ?",
            host_role: "Votre Hôte",
            concierge_role: "Concierge",
            fire: "Pompiers",
            medical: "SAMU",
            police: "Police",
            address_label: "Adresse du logement"
        },
        rules: {
            title: "Règles de vie",
            quiet: {
                title: "CALME APRÈS 22H",
                sub: "Pour le confort de tous",
                text: "Merci de baisser le volume après 22h — nos voisins sont adorables et on aimerait que ça dure 😊"
            },
            smoke: {
                title: "ESPACE NON-FUMEUR",
                sub: "On respire mieux",
                text: "La maison est non-fumeur. Si besoin, la fenêtre de la cuisine est OK pour fumer à l'extérieur."
            },
            party: {
                title: "SÉJOUR TRANQUILLE",
                sub: "Cocon plutôt que fiesta",
                text: "Ce logement est pensé pour se ressourcer. Les fêtes et événements ne sont pas autorisés."
            },
            pets: {
                title: "ANIMAUX",
                sub: "On en parle d'abord",
                text: "Les animaux ne sont pas acceptés par défaut — mais contactez-nous, on peut parfois s'arranger !"
            },
            care: {
                title: "SOIN DES LIEUX",
                sub: "Comme chez vous",
                text: "Signalez-nous tout souci ou dégât, même petit. Pas de stress, ça arrive — on préfère juste savoir."
            },
            security: {
                title: "SÉCURITÉ",
                sub: "Petit réflexe avant de partir",
                text: "Pensez à éteindre les lumières et appareils quand vous sortez. La planète (et la facture) vous remercient."
            }
        },
        guestbook: {
            title: "Livre d'or Ohana Home",
            subtitle: "Laissez une trace de votre passage ici 😁",
            form: {
                name_label: "Votre Prénom",
                name_placeholder: "Ex: Julien & Marie",
                msg_label: "Votre Message",
                msg_placeholder: "Racontez-nous votre moment préféré...",
                submit: "Graver sur le livre d'or",
                read_link: "Ou lire les messages précédents"
            },
            qr_text: "Plus cool avec votre téléphone non ?\nScannez pour écrire avec le clavier de votre mobile.",
            read_title: "Les mots des voyageurs",
            add_btn: "+ Ajouter un mot",
            empty_state: "Soyez le premier à écrire !",
            sample_entries: [
                {
                    name: "Mireille & Philippe",
                    date: "Novembre 2025",
                    text: "Un vrai coup de cœur ! La vue panoramique nous a subjugués dès notre arrivée. Tout était au-delà de nos espérances, la réalité était encore plus belle que les photos. On reviendra c'est certain !"
                },
                {
                    name: "Sarah",
                    date: "Novembre 2025",
                    text: "Je recommande les yeux fermés ! À deux pas de la gare, aussi beau que sur les photos, très bien équipé. Benjamin est super réactif et arrangeant. Merci pour tout 🙏"
                },
                {
                    name: "Lana-Jolin",
                    date: "Décembre 2025",
                    text: "Le logement est magnifique ! Meublé avec beaucoup d'attention aux détails, lits hyper confortables et vue incroyable. On a visité le Luxembourg, Strasbourg et Metz depuis ici. On recommande à 100% !"
                },
                {
                    name: "Serbah",
                    date: "Septembre 2025",
                    text: "3 nuits inoubliables ! L'appartement au 9e étage offre une vue superbe sur la rivière. Tout fonctionne parfaitement, parking privé inclus. La communication avec Benjamin était excellente. Merci pour cette hospitalité !"
                },
                {
                    name: "Ellen & Family",
                    date: "Septembre 2025",
                    text: "Parfait pour notre séjour en famille ! L'appartement est exactement comme sur les photos. Les chambres bien occultantes étaient idéales pour notre petit. Toutes les installations mentionnées étaient disponibles. On recommande !"
                }
            ]
        },
        discover: {
            title: "Nos Adresses",
            scan_route_1: "Scanner pour",
            scan_route_2: "L'Itinéraire",
            regaler: {
                title: 'Se Régaler',
                img: 'assets/img/adresses/regaler_main.png',
                hasSubcategories: true,
                subcategories: {
                    restaurants: {
                        title: 'Restaurants',
                        img: 'assets/img/adresses/cat_restaurant.png',
                        places: [
                            {
                                name: 'Les Moulins Bleus',
                                type: 'Italien',
                                review: 'Un cadre splendide en cour intérieure pour déguster la meilleure lasagne de votre vie, une véritable escapade italienne au cœur de Thionville.',
                                img: 'assets/img/adresses/resto_moulins_bleus.png',
                                maps_url: 'https://www.google.com/maps/place/Les+Moulins+Bleus+-+Thionville/@49.358979,6.1659631,17z/data=!3m1!4b1!4m6!3m5!1s0x479524d5e4a210f3:0x6a5191fb268d09c4!8m2!3d49.358979!4d6.168538!16s%2Fg%2F1tf2wr1s?entry=ttu'
                            },
                            {
                                name: 'Arsène & Clara',
                                type: 'Terrasse & Vins',
                                review: 'La terrasse incontournable pour un apéro dînatoire au coucher du soleil, où le stress de la journée s\'évapore instantanément.',
                                img: 'assets/img/adresses/resto_arsene_clara.png',
                                maps_url: 'https://www.google.com/maps/place/Restaurant+Ars%C3%A8ne+%26+Clara/@49.355,6.16897,17z/data=!3m1!4b1!4m6!3m5!1s0x47952509c1fd9b1d:0xec243f0e0955dfaa!8m2!3d49.355!4d6.16897!16s%2Fg%2F11ll5t9f06?entry=ttu'
                            },
                            {
                                name: "Le P'tit Bistro d'Ethan",
                                type: 'Bistrot Lorrain',
                                review: 'Un peu de gastronomie dans un cadre bistrot : cuisine lorraine revisitée et carte renouvelée tous les deux mois.',
                                img: 'assets/img/adresses/resto_ptit_bistro.png',
                                maps_url: 'https://www.google.com/maps/search/?api=1&query=Le+P%27tit+Bistro+d%27Ethan+Thionville'
                            }
                        ]
                    },
                    street_food: {
                        title: 'Street Food',
                        img: 'assets/img/adresses/cat_street_food.png',
                        places: [
                            {
                                name: 'Pollux',
                                type: 'Smash Burger',
                                review: 'Le meilleur smash burger de Thionville avec du bœuf français et du cheddar maturé, pour une explosion de saveurs street-food authentique.',
                                img: 'assets/img/adresses/street_pollux.jpg',
                                maps_url: 'https://www.google.com/maps/place/POLLUX/@49.3598728,6.1644123,7z/data=!4m10!1m2!2m1!1spollux!3m6!1s0x4795255cd9314d97:0xf1de455566d4ce04!8m2!3d49.3601049!4d6.1647188!15sCgZwb2xsdXhaCCIGcG9sbHV4kgEUaGFtYnVyZ2VyX3Jlc3RhdXJhbnSaASNDaFpEU1VoTk1HOW5TMFZKUTBGblNVTTVNRnA2YkVWUkVBReABAPoBBAgAEEA!16s%2Fg%2F11l5tps7x6?entry=ttu'
                            },
                            {
                                name: 'Le Class',
                                type: 'Snack',
                                review: 'Sans doute l\'un des meilleurs snacks de la région : des portions généreuses, des produits frais et un accueil au top.',
                                img: 'assets/img/adresses/street_le_class.jpg',
                                maps_url: 'https://www.google.com/maps/place/LE+CLASS/@49.3647733,6.1623457,3a,75y,90t/data=!3m8!1e2!3m6!1sAF1QipN6eVUy1ya-tlAXOe4kYC-dZh7O4rI-Hi-uyC9i!2e10!3e12!6shttps:%2F%2Flh3.googleusercontent.com%2Fp%2FAF1QipN6eVUy1ya-tlAXOe4kYC-dZh7O4rI-Hi-uyC9i%3Dw152-h86-k-no!7i3264!8i1836!4m7!3m6!1s0x47953ad4a82e54a5:0xed538aaf16b9e574!8m2!3d49.3646366!4d6.1623423!10e5!16s%2Fg%2F11c2lbptfs?entry=ttu'
                            },
                            {
                                name: 'Simply Good',
                                type: 'Snack',
                                review: 'Simplement l\'un des meilleurs snacks du coin : des recettes maîtrisées, de la fraîcheur dans l\'assiette et une convivialité qui ne déçoit jamais.',
                                img: 'assets/img/adresses/street_simply_good.png',
                                maps_url: 'https://www.google.com/maps/place/Simply+Good/@49.3577117,6.1631986,17z/data=!3m1!4b1!4m6!3m5!1s0x479525839063699d:0xc09fbb114f5b2453!8m2!3d49.3577117!4d6.1657735!16s%2Fg%2F11q35k1ppz?entry=ttu'
                            }
                        ]
                    },
                    boulangerie: {
                        title: 'Boulangerie',
                        img: 'assets/img/adresses/cat_boulangerie.png',
                        places: [
                            {
                                name: 'Boulangerie Parisienne BERNS',
                                type: 'Artisan',
                                review: 'Succombez à la brioche faite maison et au savoir-faire artisanal d\'une boulangerie qui vit au rythme de la ville.',
                                img: 'assets/img/adresses/boulangerie_berns.jpg',
                                maps_url: 'https://www.google.com/maps/place/Boulangerie+Parisienne+BERNS/@49.3618842,6.1706326,16.83z/data=!4m6!3m5!1s0x479524d5171fd4ef:0xb2db1d585fe4b653!8m2!3d49.3619922!4d6.1701664!16s%2Fg%2F1v44cxc_?entry=ttu'
                            },
                            {
                                name: 'La Fabrik des Pains Vagabonds',
                                type: 'Bio & Éthique',
                                review: 'Plus qu\'une boulangerie, un lieu de vie bio et éthique où le pain au levain naturel côtoie un café de spécialité d\'exception.',
                                img: 'assets/img/adresses/boulangerie_la_fabrik.jpg',
                                maps_url: 'https://www.google.com/maps/place/La+Fabrik+des+Pains+Vagabonds%E2%80%93+Boulangerie+%26+Caf%C3%A9+de+sp%C3%A9cialit%C3%A9/@49.3601055,6.1646082,17z/data=!3m1!4b1!4m6!3m5!1s0x4795258254b8231b:0xfcea6b29f9f43788!8m2!3d49.3601055!4d6.1646082!16s%2Fg%2F11tp30c52k?entry=ttu'
                            },
                            {
                                name: 'Pâtisserie FISCHER',
                                type: 'Viennoiserie',
                                review: 'Profitez d\'une pause douce en terrasse sur la place principale avec des viennoiseries d\'une fraîcheur garantie.',
                                img: 'assets/img/adresses/boulangerie_fischer.jpg',
                                maps_url: 'https://www.google.com/maps/place/P%C3%A2tisserie+FISCHER/@49.3582503,6.1676219,17z/data=!3m1!4b1!4m6!3m5!1s0x479524d605a163ef:0x3bd838bfc57a5779!8m2!3d49.3582503!4d6.1676219!16s%2Fg%2F1tfb0y1t?entry=ttu'
                            }
                        ]
                    }
                }
            },
            respirer: {
                title: 'Respirer',
                img: 'assets/img/adresses/respirer_main.jpg',
                hasSubcategories: true,
                subcategories: {
                    parc_urbain: {
                        title: 'Parc urbain',
                        img: 'assets/img/adresses/cat_parc_urbain.png',
                        places: [
                            {
                                name: 'Parc Wilson',
                                type: 'Parc & Kiosque',
                                review: 'Un écrin de verdure de 1,4 hectares longeant la Moselle, idéal pour une flânerie au bord de l\'eau ou écouter un concert au kiosque en été.',
                                img: 'assets/img/adresses/respirer_wilson.jpg',
                                maps_url: 'https://www.google.com/maps/place/Parc+Wilson/@49.3552561,6.1644774,17z/data=!3m1!4b1!4m6!3m5!1s0x4795252845201383:0x89da63412b1db38e!8m2!3d49.3552561!4d6.1644774!16s%2Fg%2F11ckvfjrn7?entry=ttu'
                            },
                            {
                                name: 'Parc Napoléon',
                                type: 'Roseraie & Arbres Rares',
                                review: 'L\'un des parcs préférés des Thionvillois : perdez-vous dans la roseraie aux mille senteurs et admirez des arbres aux essences rares.',
                                img: 'assets/img/adresses/respirer_napoleon.jpg',
                                maps_url: 'https://www.google.com/maps/search/?api=1&query=Parc+Napoleon+Thionville'
                            },
                            {
                                name: 'Aéroparc Yutz',
                                type: 'Loisirs & Nature',
                                review: 'Un immense poumon vert de 42 ha avec lac, jardins et aires de jeux. Le spot parfait pour un pique-nique, un barbecue ou une séance de sport en plein air.',
                                img: 'assets/img/adresses/respirer_aeroparc.png',
                                maps_url: 'https://www.google.com/maps/place/A%C3%A9roparc+Yutz/@49.3541068,6.20192,17z/data=!3m1!4b1!4m6!3m5!1s0x479525f50d700541:0x3614f994bbd44c39!8m2!3d49.3541069!4d6.2067909!16s%2Fg%2F11rr4slrlb?entry=ttu'
                            }
                        ]
                    },
                    promenade_nature: {
                        title: 'Promenade en nature',
                        img: 'assets/img/adresses/cat_promenade_nature.png',
                        places: [
                            {
                                name: 'Les Berges de la Moselle',
                                type: 'Balade & Vélo',
                                review: 'Promenez-vous sur des km en pleine nature jusqu\'au Luxembourg ! Le départ est visible depuis votre fenêtre, il suffit de suivre l\'horizon.',
                                img: 'assets/img/adresses/respirer_berges.jpg',
                                maps_url: 'https://www.google.com/maps/place/57100+Thionville/@49.3620331,6.178503,17.46z/data=!3m1!5s0x479524d00fa676db:0x3333a570d9bfe8fc!4m14!1m7!3m6!1s0x47952511ab21e71b:0x9a254d0268d15c09!2sGare+de+Thionville!8m2!3d49.3538603!4d6.1697235!16s%2Fg%2F11vcbq5p5h!3m5!1s0x47953b32ae6623fd:0xd8e852674f8d23ba!8m2!3d49.3637712!4d6.1820603!16s%2Fg%2F11y676vjxy?entry=ttu'
                            },
                            {
                                name: "Base de loisirs de Nautic'Ham",
                                type: 'Détente & Jeux',
                                review: 'Envie d’une ambiance vacances ? Plage, jeux pour enfants et verre au bord de l\'étang : le lieu idéal pour déconnecter à quelques minutes d\'ici.',
                                img: 'assets/img/adresses/respirer_nauticham.jpg',
                                maps_url: 'https://www.google.com/maps/place/Base+de+loisirs+de+Nautic\'Ham/@49.3821784,6.2032547,14z/data=!4m10!1m2!2m1!1scentre+de+loisir+bass+ham!3m6!1s0x47953d002ddbe597:0xec3d25d5678fc84e!8m2!3d49.384392!4d6.222812!15sChljZW50cmUgZGUgbG9pc2lyIGJhc3MgaGFtWhsiGWNlbnRyZSBkZSBsb2lzaXIgYmFzcyBoYW2SARBhbXVzZW1lbnRfY2VudGVymgFEQ2k5RFFVbFJRVU52WkVOb2RIbGpSamx2VDJ0YVdscFdiRFpUZW1SSVdsVjRNMkZxU2xoUmJYaEtZakowYVZSR1JSQULgAQD6AQQIABBK!16s%2Fg%2F11zj5tz7j_?entry=ttu'
                            }
                        ]
                    }
                }
            },
            explorer: {
                title: 'Explorer',
                img: 'assets/img/adresses/explorer_main.jpg',
                places: [
                    {
                        name: 'Musée de la Tour aux Puces',
                        type: 'Histoire & Archéologie',
                        review: 'Un voyage dans le temps au cœur d\'un ancien donjon médiéval : découvrez les secrets archéologiques du Pays Thionvillois pour un prix tout doux.',
                        img: 'assets/img/adresses/explorer_tour_aux_puces.jpg',
                        maps_url: 'https://www.google.com/maps/place/Mus%C3%A9e+de+la+Tour+aux+Puces/@49.3579136,6.1691047,17z/data=!3m1!4b1!4m6!3m5!1s0x479524d673e53b91:0x1ee545eb451ae07!8m2!3d49.3579136!4d6.1691047!16s%2Fg%2F11bc5fkfm9?entry=ttu'
                    },
                    {
                        name: 'Fort de Guentrange',
                        type: 'Histoire Militaire & Vue',
                        review: 'Perché sur la colline, ce géant de béton offre une vue imprenable sur la vallée et une plongée fascinante dans l\'histoire militaire de la région.',
                        img: 'assets/img/adresses/explorer_fort_guentrange.jpg',
                        maps_url: 'https://www.google.com/maps/search/?api=1&query=Fort+de+Guentrange+Thionville'
                    },
                    {
                        name: 'Escapade à Luxembourg',
                        type: 'Ville & Culture',
                        review: 'Transformez votre séjour en escapade internationale : Luxembourg-Ville et ses trésors UNESCO ne sont qu\'à 25 minutes de train direct.',
                        img: 'assets/img/adresses/explorer_luxembourg.jpg',
                        maps_url: 'https://www.google.com/maps/search/Gare+de+Luxembourg/@49.6012245,6.129576,15z/data=!3m1!4b1?entry=ttu'
                    },
                    {
                        name: 'Château de Malbrouck',
                        type: 'Médiéval & Panorama',
                        review: 'Imposant château médiéval magnifiquement restauré à la frontière (30 min). Architecture fortifiée, vues panoramiques et expositions d\'art dans un cadre d\'exception.',
                        img: 'assets/img/adresses/explorer_malbrouck.jpg',
                        maps_url: 'https://www.google.com/maps/search/?api=1&query=Chateau+de+Malbrouck'
                    }
                ]
            },
            indispensables: {
                title: 'Indispensables',
                img: 'assets/img/adresses/indispensable_main.jpg',
                hasSubcategories: true,
                subcategories: {
                    courses: {
                        title: 'Courses',
                        img: 'assets/img/adresses/cat_courses.png',
                        places: [
                            {
                                name: 'Carrefour Market',
                                type: 'Centre Ville',
                                review: 'Le supermarché de proximité idéal au cœur du centre commercial, parfait pour des courses complètes avec un large choix de produits frais.',
                                img: 'assets/img/adresses/indispensable_carrefour.jpg',
                                maps_url: 'https://www.google.com/maps/search/?api=1&query=Carrefour+Market+Thionville+Cour+des+Capucins'
                            },
                            {
                                name: 'Lidl',
                                type: 'Rapide & Éco',
                                review: 'L\'option maligne à deux pas des rails pour des courses rapides, économiques et une boulangerie en libre-service.',
                                img: 'assets/img/adresses/indispensable_lidl.jpg',
                                maps_url: 'https://www.google.com/maps/search/?api=1&query=Lidl+5+Rue+de+l+Ancienne+Gare+Thionville'
                            },
                            {
                                name: 'Centre Commercial Geric',
                                type: 'Shopping & Miam',
                                review: 'Enorme centre commercial pour faire du shopping, vos courses ou manger dans de nombreux restaurants.',
                                img: 'assets/img/adresses/indispensable_geric.jpg',
                                maps_url: 'https://www.google.com/maps/place/Centre+Commercial+Carrefour+Geric/@49.354748,6.1362505,17z/data=!3m1!4b1!4m6!3m5!1s0x47952547dab2f151:0x3050e5bbe5a1a8d!8m2!3d49.3547481!4d6.1411214!16s%2Fg%2F11cftxnlp?entry=ttu'
                            }
                        ]
                    },
                    sante: {
                        title: 'Pharmacie et santé',
                        img: 'assets/img/adresses/cat_sante.png',
                        places: [
                            {
                                name: 'Pharmacie Lafayette',
                                type: 'Grande Pharmacie',
                                review: 'Une grande pharmacie au large choix de parapharmacie à prix bas, avec une équipe disponible pour vos conseils santé.',
                                img: 'assets/img/adresses/indispensable_pharmacie.jpg',
                                maps_url: 'https://www.google.com/maps/search/?api=1&query=Pharmacie+Lafayette+des+Arcades+Thionville'
                            }
                        ]
                    },
                    mobilite: {
                        title: 'Mobilité',
                        img: 'assets/img/adresses/cat_mobilite.png',
                        places: [
                            {
                                name: 'Gare de Thionville',
                                type: 'Hub Transport',
                                review: 'Le hub central pour explorer la Grande Région, avec un accès TGV et TER direct vers Luxembourg et Metz.',
                                img: 'assets/img/adresses/indispensable_gare.png',
                                maps_url: 'https://www.google.com/maps/search/?api=1&query=Gare+de+Thionville'
                            }
                        ]
                    }
                }
            }
        },
        mantras: [
            "Respirez. Vous êtes chez vous.",
            "Prenez le temps de ne rien faire.",
            "La simplicité est la sophistication suprême.",
            "Ohana signifie famille.",
            "Écoutez le silence.",
            "Juste respirer.",
            "Accordez-vous une pause.",
            "Votre intuition a raison.",
            "Votre énergie ne ment pas.",
            "Revenez à vous.",
            "L'âme sait avant la tête.",
            "Tout commence à l'intérieur.",
            "Ce qui vous fait vibrer compte.",
            "Votre histoire s'écrit maintenant.",
            "Vous êtes plus fort que vos peurs.",
            "Le monde a besoin de votre lumière.",
            "Ce que vous cherchez vous cherche aussi.",
            "Faites-le pour vous."
        ],
        story: {
            ben: {
                title: "Benjamin",
                p1: "Hello, je m'appelle Benjamin ! Dans la vie, j'aide les entrepreneurs du bien-être à s'aligner et à trouver leur liberté.",
                p2: "Pour moi, un lieu de vie doit faire la même chose : apaiser l'esprit pour laisser la place aux belles idées.",
                p3: "J'ai voulu voir cet appartement comme une bulle hors du temps. Un mélange du mood balinais, de mon amour pour la spiritualité et la nature et d'une touche de 'Good Vibes' !",
                p4: "Vous êtes peut-être ici pour travailler, profiter en famille, explorer ou ne rien faire (c'est cool aussi 🙄), j'espère que ce lieu va vous apaiser et vous recharger ! Merci de nous avoir choisi 🙏",
                cta: "Découvrir Ohana"
            },
            ohana: {
                title: "Ohana",
                p1: "Salut, moi c'est Ohana ! 🐾",
                p2: "Papa m'a dit d'écrire un petit texte pour vous, mais je ne sais pas trop quoi dire... à part que j'adore les croquettes, jouer avec des bâtons et que j'adooooore nager !",
                p3: "Ah, et apparemment, mon prénom veut dire 'Famille' en hawaïen. Donc bienvenue dans la famille ! ❤️"
            }
        }
    },
    en: {
        hub: {
            greeting_pre: "Welcome to Ohana Home",
            greeting_sub: "Your personal guide for an unforgettable stay",
            nav: {
                stay: "Your Stay",
                addresses: "Local Gems",
                guestbook: "Guestbook",
                story: "Our Story"
            },
            wifi_label: "WIFI"
        },
        common: {
            back: "Back",
            home: "Home"
        },
        screensaver: {
            prompt: "Touch screen to start",
            ben_slide: {
                text_main: "\"Today, my office has no walls. I help entrepreneurs find their own freedom.\n\nI designed this place to inspire yours.\"",
                more_info: "MORE INFO HERE",
                qr_sub: "Yep, doing my own promo 😂",
                sig_name: "BENJAMIN",
                sig_role: "Your Host"
            },
            dog_slide: {
                title: "Hi humans! 🐾",
                intro: "If you loved your stay, a nice <b>5/5</b> is like a scratch behind the ear: it makes us so happy!",
                secret_title: "🤫 My little secret:",
                secret_content: "You might think <b>4/5</b> is a good score... but for the mean Airbnb algorithm, it's a little slap on the nose!<br><br>It takes about <b>10 perfect scores</b> to make up for a single 4/5. If we have too many, phew... we disappear from searches. No more visibility, no more guests, no more kibble... 🥺",
                cta: "So if everything was paw-fect, a 5/5 is the best way to save us!",
                feedback: "<strong>Something wrong?</strong> Tell Papa (Benjamin) by message. He runs faster than me to fix problems! And at Ohana Home we want to pamper you because you are the best travelers! ❤️"
            }
        },
        stay: {
            title: "Your Stay",
            menu: {
                checkout: "Checkout Guide",
                parking: "Parking & Wi-Fi",
                equip: "Amenities",
                rules: "House Rules",
                help: "Need Help?"
            }
        },
        parking: {
            title: "Parking & Wi-Fi",
            wifi_label: "WIFI",
            hero_label: "Private Parking",
            hero_desc_remote: "🎮 Remote provided (⚠️ please do not lose)",
            hero_desc_access: "Access: Floor -1 → Exit → Right → Right → Orange Door (red key)"
        },
        equip: {
            title: "Amenities",
            kitchen: {
                title: "Kitchen",
                oven: "Oven: Turn off and clean after use",
                dishwasher: "Dishwasher: Hold Power 3s. Red light on floor = ON",
                fridge: "Fridge: Close tightly, do not slam",
                coffee: "Coffee: Nespresso + Dolce Gusto",
                cleaning: "Cleaning: Magic sponges provided (on white shelf)"
            },
            bath: {
                title: "Bathroom",
                vent: "Condensation: Ventilate to the maximum",
                door: "Leave door open during shower if needed"
            },
            laundry: {
                title: "Laundry",
                towels: "1 Large + 1 Small towel / person",
                washer: "Washing machine: Delicate mode. Empty water tank.",
                dryer: "Dryer: Above washing machine",
                basket: "Basket: Bathroom, under the cabinet"
            },
            living: {
                title: "Living & Windows",
                sofa: "Please leave protective throws on the sofa",
                windows: "Windows: Fragile (single glazing). Handle with care.",
                curtains: "Curtains: Sound insulation + Blackout.",
                shutters: "<strong>Shutters:</strong> Rustic (scheduled for replacement). Use the small hook to block the chain so they don't roll up alone."
            },
            comfort: {
                title: "Comfort & Heating",
                heater: "<strong>Space Heater:</strong> Available in the dressing room if needed.",
                contact: "<strong>Temperature:</strong> Call us for any issue! We want you to be cozy."
            }
        },
        checkout: {
            title: "Checkout Guide",
            before: "Departure before",
            cleaning_time: "(Cleaning at 11:15)",
            steps: {
                laundry: "Dirty Laundry",
                laundry_sub: "Sheets/Towels on floor (Kitchen)",
                trash: "Trash",
                trash_sub: "Take out (Opposite the garage)",
                fridge: "Fridge",
                fridge_sub: "Empty (Leave non-perishables)",
                dishes: "Dishes",
                dishes_sub: "Start dishwasher",
                power: "Electricity",
                power_sub: "Turn everything off",
                windows: "Close",
                windows_sub: "Windows & Doors",
                keys: "Keys + Badge",
                keys_loc: "In keybox n°2",
                keys_code: "Code or Key \"POULET Benjamin\""
            }
        },
        help: {
            title: "Need Help?",
            host_role: "Your Host",
            concierge_role: "Concierge",
            fire: "Fire Dept.",
            medical: "Ambulance",
            police: "Police",
            address_label: "Home Address"
        },
        rules: {
            title: "House Rules",
            quiet: {
                title: "QUIET AFTER 10PM",
                sub: "For everyone's comfort",
                text: "Please keep the volume down after 10pm — our neighbors are lovely and we'd like to keep it that way 😊"
            },
            smoke: {
                title: "NON-SMOKING",
                sub: "Breathe easier",
                text: "The house is non-smoking. If needed, the kitchen window is OK for smoking outside."
            },
            party: {
                title: "PEACEFUL STAY",
                sub: "Cocooning, not partying",
                text: "This place is designed for recharging. Parties and events are not allowed."
            },
            pets: {
                title: "PETS",
                sub: "Let's talk first",
                text: "Pets are not accepted by default — but contact us, we can sometimes arrange something!"
            },
            care: {
                title: "TAKE CARE",
                sub: "Make yourself at home",
                text: "Please report any issue or damage, even small. No stress, it happens — we just prefer to know."
            },
            security: {
                title: "SECURITY",
                sub: "Quick check before leaving",
                text: "Remember to turn off lights and devices when you leave. The planet (and the bill) thank you."
            }
        },
        guestbook: {
            title: "Ohana Home Guestbook",
            subtitle: "Leave a trace of your stay here 😁",
            form: {
                name_label: "Your Name",
                name_placeholder: "Ex: John & Mary",
                msg_label: "Your Message",
                msg_placeholder: "Tell us about your favorite moment...",
                submit: "Etch in the Guestbook",
                read_link: "Or read previous messages"
            },
            qr_text: "Cooler with your phone right?\nScan to write using your mobile keyboard.",
            read_title: "Travelers' Words",
            add_btn: "+ Add a note",
            empty_state: "Be the first to write!",
            sample_entries: [
                {
                    name: "Mireille & Philippe",
                    date: "November 2024",
                    text: "Absolutely loved it! The panoramic view captivated us from the moment we arrived. Everything exceeded our expectations, even more beautiful than the photos. We'll definitely be back!"
                },
                {
                    name: "Sarah",
                    date: "November 2024",
                    text: "I highly recommend! Steps away from the train station, as beautiful as the photos, very well equipped. Benjamin is super responsive and accommodating. Thank you for everything 🙏"
                },
                {
                    name: "Lana-Jolin",
                    date: "December 2024",
                    text: "The place is stunning! Furnished with great attention to detail, super comfy beds and an incredible view. We visited Luxembourg, Strasbourg and Metz from here. 100% recommended!"
                },
                {
                    name: "Serbah",
                    date: "September 2024",
                    text: "3 unforgettable nights! The 9th floor apartment offers a stunning view of the river. Everything works perfectly, private parking included. Communication with Benjamin was excellent. Thank you for the hospitality!"
                },
                {
                    name: "Ellen & Family",
                    date: "September 2024",
                    text: "Perfect for our family stay! The apartment is exactly as pictured. The darkened bedrooms were ideal for our little one. All mentioned amenities were available. Highly recommended!"
                }
            ]
        },
        discover: {
            title: "Local Gems",
            scan_route_1: "Scan for",
            scan_route_2: "Directions",
            regaler: {
                title: 'Treat Yourself',
                img: 'assets/img/adresses/regaler_main.png',
                hasSubcategories: true,
                subcategories: {
                    restaurants: {
                        title: 'Restaurants',
                        img: 'assets/img/adresses/cat_restaurant.png',
                        places: [
                            {
                                name: 'Les Moulins Bleus',
                                type: 'Italian',
                                review: 'A stunning courtyard setting to enjoy the best lasagna of your life, a true Italian escape in the heart of Thionville.',
                                img: 'assets/img/adresses/resto_moulins_bleus.png',
                                maps_url: 'https://www.google.com/maps/place/Les+Moulins+Bleus+-+Thionville/@49.358979,6.1659631,17z/data=!3m1!4b1!4m6!3m5!1s0x479524d5e4a210f3:0x6a5191fb268d09c4!8m2!3d49.358979!4d6.168538!16s%2Fg%2F1tf2wr1s?entry=ttu'
                            },
                            {
                                name: 'Arsène & Clara',
                                type: 'Terrace & Wines',
                                review: 'The must-visit terrace for a sunset dinner aperitif, where the stress of the day evaporates instantly.',
                                img: 'assets/img/adresses/resto_arsene_clara.png',
                                maps_url: 'https://www.google.com/maps/place/Restaurant+Ars%C3%A8ne+%26+Clara/@49.355,6.16897,17z/data=!3m1!4b1!4m6!3m5!1s0x47952509c1fd9b1d:0xec243f0e0955dfaa!8m2!3d49.355!4d6.16897!16s%2Fg%2F11ll5t9f06?entry=ttu'
                            },
                            {
                                name: "Le P'tit Bistro d'Ethan",
                                type: 'Lorraine Bistro',
                                review: 'A touch of gastronomy in a bistro setting: revisited Lorraine cuisine with a menu renewed every two months.',
                                img: 'assets/img/adresses/resto_ptit_bistro.png',
                                maps_url: 'https://www.google.com/maps/search/?api=1&query=Le+P%27tit+Bistro+d%27Ethan+Thionville'
                            }
                        ]
                    },
                    street_food: {
                        title: 'Street Food',
                        img: 'assets/img/adresses/cat_street_food.png',
                        places: [
                            {
                                name: 'Pollux',
                                type: 'Smash Burger',
                                review: 'The best smash burger in Thionville with French beef and aged cheddar, for a burst of authentic street-food flavors.',
                                img: 'assets/img/adresses/street_pollux.jpg',
                                maps_url: 'https://www.google.com/maps/place/POLLUX/@49.3598728,6.1644123,7z/data=!4m10!1m2!2m1!1spollux!3m6!1s0x4795255cd9314d97:0xf1de455566d4ce04!8m2!3d49.3601049!4d6.1647188!15sCgZwb2xsdXhaCCIGcG9sbHV4kgEUaGFtYnVyZ2VyX3Jlc3RhdXJhbnSaASNDaFpEU1VoTk1HOW5TMFZKUTBGblNVTTVNRnA2YkVWUkVBReABAPoBBAgAEEA!16s%2Fg%2F11l5tps7x6?entry=ttu'
                            },
                            {
                                name: 'Le Class',
                                type: 'Snack Bar',
                                review: 'Undoubtedly one of the best snack bars in the region: generous portions, fresh products, and top-notch welcome.',
                                img: 'assets/img/adresses/street_le_class.jpg',
                                maps_url: 'https://www.google.com/maps/place/LE+CLASS/@49.3647733,6.1623457,3a,75y,90t/data=!3m8!1e2!3m6!1sAF1QipN6eVUy1ya-tlAXOe4kYC-dZh7O4rI-Hi-uyC9i!2e10!3e12!6shttps:%2F%2Flh3.googleusercontent.com%2Fp%2FAF1QipN6eVUy1ya-tlAXOe4kYC-dZh7O4rI-Hi-uyC9i%3Dw152-h86-k-no!7i3264!8i1836!4m7!3m6!1s0x47953ad4a82e54a5:0xed538aaf16b9e574!8m2!3d49.3646366!4d6.1623423!10e5!16s%2Fg%2F11c2lbptfs?entry=ttu'
                            },
                            {
                                name: 'Simply Good',
                                type: 'Snack Bar',
                                review: 'Simply one of the best local snack spots: mastered recipes, freshness on the plate, and friendliness that never disappoints.',
                                img: 'assets/img/adresses/street_simply_good.png',
                                maps_url: 'https://www.google.com/maps/place/Simply+Good/@49.3577117,6.1631986,17z/data=!3m1!4b1!4m6!3m5!1s0x479525839063699d:0xc09fbb114f5b2453!8m2!3d49.3577117!4d6.1657735!16s%2Fg%2F11q35k1ppz?entry=ttu'
                            }
                        ]
                    },
                    boulangerie: {
                        title: 'Bakery',
                        img: 'assets/img/adresses/cat_boulangerie.png',
                        places: [
                            {
                                name: 'Boulangerie Parisienne BERNS',
                                type: 'Artisan',
                                review: 'Indulge in the homemade brioche and artisanal know-how of a bakery that beats to the rhythm of the city.',
                                img: 'assets/img/adresses/boulangerie_berns.jpg',
                                maps_url: 'https://www.google.com/maps/place/Boulangerie+Parisienne+BERNS/@49.3618842,6.1706326,16.83z/data=!4m6!3m5!1s0x479524d5171fd4ef:0xb2db1d585fe4b653!8m2!3d49.3619922!4d6.1701664!16s%2Fg%2F1v44cxc_?entry=ttu'
                            },
                            {
                                name: 'La Fabrik des Pains Vagabonds',
                                type: 'Organic & Ethical',
                                review: 'More than a bakery, an organic and ethical living space where natural sourdough bread rubs shoulders with exceptional specialty coffee.',
                                img: 'assets/img/adresses/boulangerie_la_fabrik.jpg',
                                maps_url: 'https://www.google.com/maps/place/La+Fabrik+des+Pains+Vagabonds%E2%80%93+Boulangerie+%26+Caf%C3%A9+de+sp%C3%A9cialit%C3%A9/@49.3601055,6.1646082,17z/data=!3m1!4b1!4m6!3m5!1s0x4795258254b8231b:0xfcea6b29f9f43788!8m2!3d49.3601055!4d6.1646082!16s%2Fg%2F11tp30c52k?entry=ttu'
                            },
                            {
                                name: 'Pâtisserie FISCHER',
                                type: 'Viennoiserie',
                                review: 'Enjoy a sweet break on the terrace in the main square with guaranteed fresh pastries.',
                                img: 'assets/img/adresses/boulangerie_fischer.jpg',
                                maps_url: 'https://www.google.com/maps/place/P%C3%A2tisserie+FISCHER/@49.3582503,6.1676219,17z/data=!3m1!4b1!4m6!3m5!1s0x479524d605a163ef:0x3bd838bfc57a5779!8m2!3d49.3582503!4d6.1676219!16s%2Fg%2F1tfb0y1t?entry=ttu'
                            }
                        ]
                    }
                }
            },
            respirer: {
                title: 'Breathe',
                img: 'assets/img/adresses/respirer_main.jpg',
                hasSubcategories: true,
                subcategories: {
                    parc_urbain: {
                        title: 'Urban Park',
                        img: 'assets/img/adresses/cat_parc_urbain.png',
                        places: [
                            {
                                name: 'Parc Wilson',
                                type: 'Park & Kiosk',
                                review: 'A green setting of 1.4 hectares along the Moselle, ideal for a stroll by the water or listening to a concert at the kiosk in summer.',
                                img: 'assets/img/adresses/respirer_wilson.jpg',
                                maps_url: 'https://www.google.com/maps/place/Parc+Wilson/@49.3552561,6.1644774,17z/data=!3m1!4b1!4m6!3m5!1s0x4795252845201383:0x89da63412b1db38e!8m2!3d49.3552561!4d6.1644774!16s%2Fg%2F11ckvfjrn7?entry=ttu'
                            },
                            {
                                name: 'Parc Napoléon',
                                type: 'Rose Garden & Rare Trees',
                                review: 'One of the locals\' favorite parks: lose yourself in the rose garden with a thousand scents and admire trees of rare species.',
                                img: 'assets/img/adresses/respirer_napoleon.jpg',
                                maps_url: 'https://www.google.com/maps/search/?api=1&query=Parc+Napoleon+Thionville'
                            },
                            {
                                name: 'Aéroparc Yutz',
                                type: 'Leisure & Nature',
                                review: 'A huge green lung of 42 ha with lake, gardens, and playgrounds. The perfect spot for a picnic, a barbecue, or an outdoor sports session.',
                                img: 'assets/img/adresses/respirer_aeroparc.png',
                                maps_url: 'https://www.google.com/maps/place/A%C3%A9roparc+Yutz/@49.3541068,6.20192,17z/data=!3m1!4b1!4m6!3m5!1s0x479525f50d700541:0x3614f994bbd44c39!8m2!3d49.3541069!4d6.2067909!16s%2Fg%2F11rr4slrlb?entry=ttu'
                            }
                        ]
                    },
                    promenade_nature: {
                        title: 'Nature Walks',
                        img: 'assets/img/adresses/cat_promenade_nature.png',
                        places: [
                            {
                                name: 'Les Berges de la Moselle',
                                type: 'Walking & Cycling',
                                review: 'Walk for kilometers in nature all the way to Luxembourg! The start is visible from your window, just follow the horizon.',
                                img: 'assets/img/adresses/respirer_berges.jpg',
                                maps_url: 'https://www.google.com/maps/place/57100+Thionville/@49.3620331,6.178503,17.46z/data=!3m1!5s0x479524d00fa676db:0x3333a570d9bfe8fc!4m14!1m7!3m6!1s0x47952511ab21e71b:0x9a254d0268d15c09!2sGare+de+Thionville!8m2!3d49.3538603!4d6.1697235!16s%2Fg%2F11vcbq5p5h!3m5!1s0x47953b32ae6623fd:0xd8e852674f8d23ba!8m2!3d49.3637712!4d6.1820603!16s%2Fg%2F11y676vjxy?entry=ttu'
                            },
                            {
                                name: "Base de loisirs de Nautic'Ham",
                                type: 'Relaxation & Games',
                                review: 'Fancy a holiday vibe? Beach, games for children, and a drink by the pond: the ideal place to disconnect just a few minutes from here.',
                                img: 'assets/img/adresses/respirer_nauticham.jpg',
                                maps_url: 'https://www.google.com/maps/place/Base+de+loisirs+de+Nautic\'Ham/@49.3821784,6.2032547,14z/data=!4m10!1m2!2m1!1scentre+de+loisir+bass+ham!3m6!1s0x47953d002ddbe597:0xec3d25d5678fc84e!8m2!3d49.384392!4d6.222812!15sChljZW50cmUgZGUgbG9pc2lyIGJhc3MgaGFtWhsiGWNlbnRyZSBkZSBsb2lzaXIgYmFzcyBoYW2SARBhbXVzZW1lbnRfY2VudGVymgFEQ2k5RFFVbFJRVU52WkVOb2RIbGpSamx2VDJ0YVdscFdiRFpUZW1SSVdsVjRNMkZxU2xoUmJYaEtZakowYVZSR1JSQULgAQD6AQQIABBK!16s%2Fg%2F11zj5tz7j_?entry=ttu'
                            }
                        ]
                    }
                }
            },
            explorer: {
                title: 'Explore',
                img: 'assets/img/adresses/explorer_main.jpg',
                places: [
                    {
                        name: 'Musée de la Tour aux Puces',
                        type: 'History & Archeology',
                        review: 'A journey through time in the heart of an ancient medieval dungeon: discover the archaeological secrets of the Thionville region for a very soft price.',
                        img: 'assets/img/adresses/explorer_tour_aux_puces.jpg',
                        maps_url: 'https://www.google.com/maps/place/Mus%C3%A9e+de+la+Tour+aux+Puces/@49.3579136,6.1691047,17z/data=!3m1!4b1!4m6!3m5!1s0x479524d673e53b91:0x1ee545eb451ae07!8m2!3d49.3579136!4d6.1691047!16s%2Fg%2F11bc5fkfm9?entry=ttu'
                    },
                    {
                        name: 'Fort de Guentrange',
                        type: 'Military History & View',
                        review: 'Perched on the hill, this concrete giant offers breathtaking views of the valley and a fascinating immersion into the region\'s military history.',
                        img: 'assets/img/adresses/explorer_fort_guentrange.jpg',
                        maps_url: 'https://www.google.com/maps/search/?api=1&query=Fort+de+Guentrange+Thionville'
                    },
                    {
                        name: 'Escapade à Luxembourg',
                        type: 'City & Culture',
                        review: 'Turn your stay into an international getaway: Luxembourg City and its UNESCO treasures are only 25 minutes by direct train.',
                        img: 'assets/img/adresses/explorer_luxembourg.jpg',
                        maps_url: 'https://www.google.com/maps/search/Gare+de+Luxembourg/@49.6012245,6.129576,15z/data=!3m1!4b1?entry=ttu'
                    },
                    {
                        name: 'Château de Malbrouck',
                        type: 'Medieval & Panorama',
                        review: 'Imposing medieval castle beautifully restored on the border (30 min). Fortified architecture, panoramic views, and art exhibitions in an exceptional setting.',
                        img: 'assets/img/adresses/explorer_malbrouck.jpg',
                        maps_url: 'https://www.google.com/maps/search/?api=1&query=Chateau+de+Malbrouck'
                    }
                ]
            },
            indispensables: {
                title: 'Essentials',
                img: 'assets/img/adresses/indispensable_main.jpg',
                hasSubcategories: true,
                subcategories: {
                    courses: {
                        title: 'Groceries',
                        img: 'assets/img/adresses/cat_courses.png',
                        places: [
                            {
                                name: 'Carrefour Market',
                                type: 'City Center',
                                review: 'The ideal local supermarket in the heart of the shopping center, perfect for complete shopping with a wide choice of fresh products.',
                                img: 'assets/img/adresses/indispensable_carrefour.jpg',
                                maps_url: 'https://www.google.com/maps/search/?api=1&query=Carrefour+Market+Thionville+Cour+des+Capucins'
                            },
                            {
                                name: 'Lidl',
                                type: 'Quick & Eco',
                                review: 'The smart option just steps from the tracks for quick, economical shopping and a self-service bakery.',
                                img: 'assets/img/adresses/indispensable_lidl.jpg',
                                maps_url: 'https://www.google.com/maps/search/?api=1&query=Lidl+5+Rue+de+l+Ancienne+Gare+Thionville'
                            },
                            {
                                name: 'Centre Commercial Geric',
                                type: 'Shopping & Food',
                                review: 'Major shopping center for shopping, groceries, or dining in one of the many restaurants.',
                                img: 'assets/img/adresses/indispensable_geric.jpg',
                                maps_url: 'https://www.google.com/maps/place/Centre+Commercial+Carrefour+Geric/@49.354748,6.1362505,17z/data=!3m1!4b1!4m6!3m5!1s0x47952547dab2f151:0x3050e5bbe5a1a8d!8m2!3d49.3547481!4d6.1411214!16s%2Fg%2F11cftxnlp?entry=ttu'
                            }
                        ]
                    },
                    sante: {
                        title: 'Pharmacy & Health',
                        img: 'assets/img/adresses/cat_sante.png',
                        places: [
                            {
                                name: 'Pharmacie Lafayette',
                                type: 'Large Pharmacy',
                                review: 'A large pharmacy with a wide choice of parapharmacy at low prices, with a team available to give health advice.',
                                img: 'assets/img/adresses/indispensable_pharmacie.jpg',
                                maps_url: 'https://www.google.com/maps/search/?api=1&query=Pharmacie+Lafayette+des+Arcades+Thionville'
                            }
                        ]
                    },
                    mobilite: {
                        title: 'Mobility',
                        img: 'assets/img/adresses/cat_mobilite.png',
                        places: [
                            {
                                name: 'Gare de Thionville',
                                type: 'Transport Hub',
                                review: 'The central hub for exploring the Greater Region, with direct TGV and TER access to Luxembourg and Metz.',
                                img: 'assets/img/adresses/indispensable_gare.png',
                                maps_url: 'https://www.google.com/maps/search/?api=1&query=Gare+de+Thionville'
                            }
                        ]
                    }
                }
            }
        },
        mantras: [
            "Breathe. You are home.",
            "Take time to do nothing.",
            "Simplicity is the ultimate sophistication.",
            "Ohana means family.",
            "Listen to the silence.",
            "Just breathe.",
            "Give yourself a break.",
            "Your intuition is right.",
            "Your energy doesn't lie.",
            "Come back to yourself.",
            "The soul knows before the mind.",
            "Everything starts from within.",
            "What moves you matters.",
            "Your story is being written now.",
            "You are stronger than your fears.",
            "The world needs your light.",
            "What you seek is seeking you.",
            "Do it for you."
        ],
        story: {
            ben: {
                title: "Benjamin",
                p1: "Hello, I'm Benjamin! In life, I help wellness entrepreneurs align themselves and find their freedom.",
                p2: "For me, a living space should do the same: soothe the mind to make room for beautiful ideas.",
                p3: "I wanted to see this apartment as a bubble out of time. A mix of Balinese mood, my love for spirituality and nature, and a touch of 'Good Vibes'!",
                p4: "Whether you are here to work, enjoy family time, explore, or do nothing (that's cool too 🙄), I hope this place soothes and recharges you! Thank you for choosing us 🙏",
                cta: "Discover Ohana"
            },
            ohana: {
                title: "Ohana",
                p1: "Hi, I'm Ohana! 🐾",
                p2: "Dad told me to write a little text for you, but I don't really know what to say... except that I love kibble, playing with sticks, and I looooove swimming!",
                p3: "Ah, and apparently, my name means 'Family' in Hawaiian. So welcome to the family! ❤️"
            }
        }
    }
};
